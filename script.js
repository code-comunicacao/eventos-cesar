...
function mapRowToEvent(row, idx, rowIndex) {
  ...
  const galleryUrls = extractUrls(get('Galeria'));
...
function initDashboardPanel() {
  ...
  function computeMetrics(events) {
    ...
    const buildingTotalMap = new Map();
    const buildingSalaMap = new Map();
    ...
    events.forEach((ev) => {
      ...
      const building = String(ev.venue || '').trim();
      const sala = String(ev.sala || '').trim();
      if (building) {
        buildingTotalMap.set(building, (buildingTotalMap.get(building) || 0) + 1);
        if (sala) {
          if (!buildingSalaMap.has(building)) buildingSalaMap.set(building, new Map());
          const salaMap = buildingSalaMap.get(building);
          salaMap.set(sala, (salaMap.get(sala) || 0) + 1);
        }
      }
    ...
    // Top 5 prédios por nº de eventos; dentro de cada um, as 3 salas mais
    // usadas viram segmentos nomeados e o resto (salas menos frequentes +
    // eventos sem sala definida) cai em "Outras salas" — assim a barra
    // sempre soma 100% mesmo com nomes de sala digitados de forma livre.
    const buildingEntries = Array.from(buildingTotalMap.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([building, buildingTotal]) => {
        const salaMap = buildingSalaMap.get(building) || new Map();
        const topSalas = Array.from(salaMap.entries()).sort((a, b) => b[1] - a[1]).slice(0, 3);
        const namedTotal = topSalas.reduce((sum, [, v]) => sum + v, 0);
        const outros = buildingTotal - namedTotal;
        const segments = topSalas.map(([name, value]) => ({ name, value }));
        if (outros > 0) segments.push({ name: 'Outras salas', value: outros, isOther: true });
        return { building, total: buildingTotal, segments };
      });
  ...
  // Aqui a cor é por RANKING dentro do prédio (sala mais usada / 2ª / 3ª /
  // outras), não por identidade da sala — os nomes de sala variam livremente
  // de prédio pra prédio (e têm bastante erro de digitação), então uma cor
  // fixa por nome não faria sentido. O nome real de cada sala aparece no
  // rótulo do segmento e no aria-label/title.
  const ROOM_RANK_FILLS = ['var(--treemap-1)', 'var(--treemap-2)', 'var(--treemap-3)'];
  const ROOM_OTHER_FILL = 'var(--treemap-other)';

  function renderStackedBar(elId, legendId, buildingEntries) {
    const wrap = document.getElementById(elId);
    ...
    buildingEntries.forEach(({ building, total, segments }) => {
      const row = document.createElement('div');
      row.className = 'stackbar-row';

      const head = document.createElement('div');
      head.className = 'stackbar-row-head';
      const nameEl = document.createElement('span');
      nameEl.className = 'stackbar-row-name';
      nameEl.textContent = building;
      const totalEl = document.createElement('span');
      totalEl.className = 'stackbar-row-total';
      totalEl.textContent = `${total} evento${total > 1 ? 's' : ''}`;
      head.append(nameEl, totalEl);

      const track = document.createElement('div');
      track.className = 'stackbar-track';
      segments.forEach((seg, i) => {
        const pct = total ? Math.round((seg.value / total) * 100) : 0;
        const segEl = document.createElement('span');
        segEl.className = 'stackbar-seg';
        segEl.style.width = `${pct}%`;
        segEl.style.background = seg.isOther ? ROOM_OTHER_FILL : ROOM_RANK_FILLS[i];
        segEl.style.color = seg.isOther ? 'var(--off-white)' : 'var(--ink)';
        const label = `${seg.name}: ${seg.value} evento${seg.value > 1 ? 's' : ''} (${pct}%)`;
        segEl.setAttribute('aria-label', label);
        segEl.title = label;
        segEl.tabIndex = 0;

        const segLabel = document.createElement('span');
        segLabel.className = 'stackbar-seg-label';
        segLabel.textContent = `${seg.name} · ${pct}%`;
    ...
    if (legend) {
      const legendDefs = [
        { label: 'Sala mais usada', fill: ROOM_RANK_FILLS[0] },
        { label: '2ª mais usada', fill: ROOM_RANK_FILLS[1] },
        { label: '3ª mais usada', fill: ROOM_RANK_FILLS[2] },
        { label: 'Outras salas', fill: ROOM_OTHER_FILL },
      ];
      legendDefs.forEach(({ label, fill }) => {
        ...
        item.append(dot, textEl);
        legend.appendChild(item);
      });
    }

    // some com o rótulo dos segmentos estreitos demais pra caber com folga —
    // o valor continua acessível pelo aria-label/title
    requestAnimationFrame(() => {
      wrap.querySelectorAll('.stackbar-seg').forEach((seg) => {
        seg.classList.toggle('is-compact', seg.offsetWidth < 56);
      });
    });
  }
  ...
  function render(events, noticeKind) {
    ...
    renderStackedBar('roomsStackChart', 'roomsStackLegend', metrics.buildingEntries);
...
/* ============================================================
   PLANILHA → CALENDÁRIO + PAINEL + GALERIA
   Busca a planilha uma única vez e alimenta os três com os
...
(function initEventsFeed() {
  ...
  if (!calendar && !dashboard && !gallery) return;
  ...
  async function load() {
    ...
    if (gallery) gallery.render(events);
  }
...
/* ============================================================
   ...
   Combina GALLERY_DATA (fotos curadas à mão, ver topo do arquivo)
   com as fotos da coluna "Galeria" da planilha de eventos — uma
   por link encontrado na célula, no ano do evento.
   ============================================================ */
function buildGalleryFromEvents(events) {
  const items = [];
  events.forEach((ev) => {
    if (!ev.galleryUrls || !ev.galleryUrls.length) return;
    const d = parseISODate(ev.date);
    const month = MONTHS_PT[d.getMonth()];
    ev.galleryUrls.forEach((url) => {
      items.push({
        year: d.getFullYear(),
        title: ev.title,
...
function initGallery() {
  ...
  function renderTabs() {
    years = Array.from(new Set(allItems.map((p) => p.year))).sort((a, b) => b - a);
    ...
    years.forEach((year) => {
      ...
      if (year === activeYear) tab.classList.add('is-active');
  ...
  function renderGrid() {
    activeItems = allItems.filter((p) => p.year === activeYear);
    grid.innerHTML = '';
  ...
  function render(events) {
    allItems = GALLERY_DATA.concat(buildGalleryFromEvents(events || []));
    renderTabs();
    renderGrid();
  }
